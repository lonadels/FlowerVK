...
class MainFunctions 
{
    ...
    public function stats(){
        ...
        return $this->mainModule->thread(function()use($progressName){
            ...
            use(    
                ...
                $a = Regex::of("\\b([a-z-а-я0-9]+)\\b")->with($item->text);
                $b = Regex::of("\\[".($fid>0?"id":"public")."*\\{$fid}|.*\\]")->with($item->text);
    ...
    public function graffiti(){
        ...
        return $this->mainModule->thread(function()use($progressName){          
            ...
            $file = (object) $this->vk->method( ["docs", "save"], new MethodParams([ "file" => $file ]) )->response;
            ...
            if( $this->vk->method( ["messages", "send"], new MethodParams([ "random_id"=>time(), "attachment" => "doc" . $fileData->owner_id . "_" . $fileData->id, "peer_id" => $peer_id ]) )->error )
    ...
    public function voice(){
        ...
        return $this->mainModule->thread(function()use($progressName){          
            ...
            $file = json_decode( $content )->file;
            $file = (object) $this->vk->method( ["docs", "save"], new MethodParams([ "file" => $file ]) )->response[ 0 ];
